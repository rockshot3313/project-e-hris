        ...
        {{--                <a href="" class="flex items-center ml-auto text-primary"> <i data-lucide="map-pin" class="w-4 h-4 mr-2"></i> Tracking Info </a>--}}
                    ...
                    @if($count == 0)
                        ...
                        @forelse($getAttachments as $attachments)
                            <div class="flex items-center mt-3"> <i data-lucide="paperclip" class="w-4 h-4 text-slate-500 mr-2"></i><a href="/documents/download-documents/{{ $attachments->path }}" target="_blank" class="underline decoration-dotted ml-1 text-toldok2">{{ $attachments->name }}</a></div>
                        @empty
