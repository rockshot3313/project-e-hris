...
use Illuminate\Support\Facades\Auth;
    ...
    function createNotification($subject,$subject_id,$sender_type,$sender_id,$target_id,$target_type,$notif_content,$purpose)
    {
        $add_members = [
            'subject' =>  $subject,
    ...
    //check if the present in the tblpanes
    function check_panels($panel_id,$jobref)
    {
         $validate_panel = tblpanels::where('panel_id',$panel_id)->where('available_ref',$jobref)
                            ->where('active',true)->get();

                foreach($validate_panel as $validated)
                {
                    if(($validated->panel_id == $panel_id) && ($validated->available_ref == $jobref))
                        {
                            return true;
                        }
                        else
...
//=============================================================================
    ...
    function get_employee_idp()
    {
        ...
        $emp_name = tblemployee::whereNotNull('agencyid')->get();
