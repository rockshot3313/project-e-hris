...
// if($getApplicants->rate_sched)
        ...
        if($getApplicants->count() > 0){
            ...
            foreach ($getApplicants as $getApplicant) {
               ...
               $rate_btn = '';
                    $sched_date = $getApplicant->rate_sched;
                    $rate_date = (new Carbon)->parse($sched_date);
                    $date_today =  Carbon::now()->addMinutes(720)->shiftTimezone('Asia/Manila'); // today

                    $diff_of_days = $date_today->diffInHours($rate_date);

                    // dd($rate_date, ''.$date_today);
                    if($rate_date <= $date_today){
                        $rate_btn = '<div class="flex justify-center items-center">
                                        <div class="w-8 h-8 rounded-full flex items-center justify-center border dark:border-darkmode-400 ml-2 text-slate-400 zoom-in tooltip dropdown" title="Rate">
                                            ...
                                            class="flex justify-center items-center text-primary rate_Icon"
                                            href="javascript:;">
                                ...
                                <td>
                                    ...
                                    <div class="flex items-center">
                                        <div class="w-2 h-2 bg-pending rounded-full mr-3"></div>
                                            <span class="truncate">'.$getApplicant->get_stat->name.'</span>

                                        <div class="ml-auto cursor-pointer tooltip">

                                        <p id="timer" class = "timer text-xs">
    ...
    public function filterByPosition(Request $request, $id){
        // dd($request->all(), $id);
        $position = [];
        ...
        foreach ($filterCriteria as $criteria) {
            ...
            $sumOf_ratedArea = '';
            if($ratedArea){
               $sumOf_ratedAreass = ratedArea_model::where('criteria_id', $criteria->id)->where('position_id', $id)->where('applicant_id', $request->applicantid)->where('rated_by', auth()->user()->employee)->sum('rate');
    ...
    public function showRatingArea(Request $request, $id){
        ...
        // dd($areass);$rated_id = ratedArea_model::where('criteria_id', $id)->where('position_id', $request->positionID)->where('applicant_id',$request->applicantID)->where('rated_by', auth()->user()->employee)
