...
class AdminController extends Controller
{
    ...
    public function load_employee(Request $request){
        ...
        if($request->user_id){
            $employee = tblemployee::with('getHRdetails','getUsername')->has('getUsername')->where('agencyid',$request->user_id)->where('active',1)->first();
        }else{
            $employee = tblemployee::with('getHRdetails','getUsername')->has('getUsername')->where('active',1)->get();
        }
    ...
    public function load_user_priv(Request $request){
        ...
        $user_priv = doc_user_privilege::with('getModule')->where('user_id', $user_id)->where('active',1)->get();
    ...
    public function update_user_priv(Request $request){
            ...
            if($request->has('multiple_user')) {
                ...
                foreach($request->multiple_user as $key => $user_id){
                    ...
                    if($request->has('readList')) {
                        $getModuleschecked = doc_user_privilege::where('user_id',$user_id)->whereIn('module_id',$request->readList )->where('active',1)->get();
                        foreach ($getModuleschecked as $checked) {
                            $update_priv = [
                                'read' => '1',
                            ];
                            doc_user_privilege::where(['id' =>  $checked->id])->first()->update($update_priv);
                                }

                        $getModulesnotchecked = doc_user_privilege::where('user_id',$user_id)->whereNotIn('module_id',$request->readList )->where('active',1)->get();
                        foreach ($getModulesnotchecked as $notchecked) {
                            $update_priv = [
                                'read' => '0',
                            ];
                            doc_user_privilege::where(['id' =>  $notchecked->id])->first()->update($update_priv);
                                }
                            }else{

                                $getModulesnotchecked = doc_user_privilege::where('user_id',$user_id)->where('active',1)->get();
                                foreach ($getModulesnotchecked as $notchecked) {
                                    $update_priv = [
                                        'read' => '0',
                                    ];
                            ...
                            if($request->has('writeList')) {
                                $getModuleschecked = doc_user_privilege::where('user_id',$user_id)->whereIn('module_id',$request->writeList )->where('active',1)->get();
                                foreach ($getModuleschecked as $checked) {
                                    $update_priv = [
                                        'write' => '1',
                                    ];
                                ...
                                $getModulesnotchecked = doc_user_privilege::where('user_id',$user_id)->whereNotIn('module_id',$request->writeList )->where('active',1)->get();
                                foreach ($getModulesnotchecked as $notchecked) {
                                    $update_priv = [
                                        'write' => '0',
                                    ];
                                    ...
                                    if($request->has('createList')) {
                                        $getModuleschecked = doc_user_privilege::where('user_id',$user_id)->whereIn('module_id',$request->createList )->where('active',1)->get();
                                        foreach ($getModuleschecked as $checked) {
                                            $update_priv = [
                                                'create' => '1',
                                            ];
                                        ...
                                        $getModulesnotchecked = doc_user_privilege::where('user_id',$user_id)->whereNotIn('module_id',$request->createList )->where('active',1)->get();
                                        foreach ($getModulesnotchecked as $notchecked) {
                                            $update_priv = [
                                                'create' => '0',
                                            ];
                                            ...
                                            if($request->has('dtdeleteList')) {
                                                //$getModules = doc_user_privilege::where('user_id',$request->user_id)->get();
                                                $getModuleschecked = doc_user_privilege::where('user_id',$user_id)->whereIn('module_id',$request->dtdeleteList )->where('active',1)->get();
                                                foreach ($getModuleschecked as $checked) {
                                                    $update_priv = [
                                                        'delete' => '1',
                                                    ];
                                                ...
                                                $getModulesnotchecked = doc_user_privilege::where('user_id',$user_id)->whereNotIn('module_id',$request->dtdeleteList )->where('active',1)->get();
                                                foreach ($getModulesnotchecked as $notchecked) {
                                                    $update_priv = [
                                                        'delete' => '0',
                                                    ];
                                                    ...
                                                    if($request->has('dtimportList')) {
                                                        //$getModules = doc_user_privilege::where('user_id',$request->user_id)->get();
                                                        $getModuleschecked = doc_user_privilege::where('user_id',$user_id)->whereIn('module_id',$request->dtimportList )->where('active',1)->get();
                                                        foreach ($getModuleschecked as $checked) {
                                                            $update_priv = [
                                                                'import' => '1',
                                                            ];
                                                        ...
                                                        $getModulesnotchecked = doc_user_privilege::where('user_id',$user_id)->whereNotIn('module_id',$request->dtimportList )->where('active',1)->get();
                                                        foreach ($getModulesnotchecked as $notchecked) {
                                                            $update_priv = [
                                                                'import' => '0',
                                                            ];
                                                            ...
                                                            if($request->has('dtexportList')) {
                                                                //$getModules = doc_user_privilege::where('user_id',$request->user_id)->get();
                                                                $getModuleschecked = doc_user_privilege::where('user_id',$user_id)->whereIn('module_id',$request->dtexportList )->where('active',1)->get();
                                                                foreach ($getModuleschecked as $checked) {
                                                                    $update_priv = [
                                                                        'export' => '1',
                                                                    ];
                                                                ...
                                                                $getModulesnotchecked = doc_user_privilege::where('user_id',$user_id)->whereNotIn('module_id',$request->dtexportList )->where('active',1)->get();
                                                                foreach ($getModulesnotchecked as $notchecked) {
                                                                    $update_priv = [
                                                                        'export' => '0',
                                                                    ];
                                                            ...
                                                            if($request->has('dtprintList')) {
                                                                //$getModules = doc_user_privilege::where('user_id',$request->user_id)->get();
                                                                $getModuleschecked = doc_user_privilege::where('user_id',$user_id)->whereIn('module_id',$request->dtprintList )->where('active',1)->get();
                                                                foreach ($getModuleschecked as $checked) {
                                                                    $update_priv = [
                                                                        'print' => '1',
                                                                    ];
                                                                ...
                                                                $getModulesnotchecked = doc_user_privilege::where('user_id',$user_id)->whereNotIn('module_id',$request->dtprintList )->where('active',1)->get();
                                                                foreach ($getModulesnotchecked as $notchecked) {
                                                                    $update_priv = [
                                                                        'print' => '0',
                                                                    ];
        ...
        }else
        {
            ...
            if($request->has('readList')) {
                //$getModules = doc_user_privilege::where('user_id',$request->user_id)->get();
                $getModuleschecked = doc_user_privilege::where('user_id',$request->user_id)->whereIn('module_id',$request->readList )->where('active',1)->get();
                foreach ($getModuleschecked as $checked) {
                    $update_priv = [
                        'read' => '1',
                    ];
                ...
                $getModulesnotchecked = doc_user_privilege::where('user_id',$request->user_id)->whereNotIn('module_id',$request->readList )->where('active',1)->get();
                foreach ($getModulesnotchecked as $notchecked) {
                    $update_priv = [
                        'read' => '0',
                    ];
                    doc_user_privilege::where(['id' =>  $notchecked->id])->first()->update($update_priv);
                        ...
                        $getModulesnotchecked = doc_user_privilege::where('user_id',$request->user_id)->where('active',1)->get();
                        foreach ($getModulesnotchecked as $notchecked) {
                            $update_priv = [
                                'read' => '0',
                            ];
                    ...
                    if($request->has('writeList')) {
                        //$getModules = doc_user_privilege::where('user_id',$request->user_id)->get();
                        $getModuleschecked = doc_user_privilege::where('user_id',$request->user_id)->whereIn('module_id',$request->writeList )->where('active',1)->get();
                        foreach ($getModuleschecked as $checked) {
                            $update_priv = [
                                'write' => '1',
                            ];
                        ...
                        $getModulesnotchecked = doc_user_privilege::where('user_id',$request->user_id)->whereNotIn('module_id',$request->writeList )->where('active',1)->get();
                        foreach ($getModulesnotchecked as $notchecked) {
                            $update_priv = [
                                'write' => '0',
                            ];
                            ...
                            if($request->has('createList')) {
                                //$getModules = doc_user_privilege::where('user_id',$request->user_id)->get();
                                $getModuleschecked = doc_user_privilege::where('user_id',$request->user_id)->whereIn('module_id',$request->createList )->where('active',1)->get();
                                foreach ($getModuleschecked as $checked) {
                                    $update_priv = [
                                        'create' => '1',
                                    ];
                                ...
                                $getModulesnotchecked = doc_user_privilege::where('user_id',$request->user_id)->whereNotIn('module_id',$request->createList )->where('active',1)->get();
                                foreach ($getModulesnotchecked as $notchecked) {
                                    $update_priv = [
                                        'create' => '0',
                                    ];
                                    ...
                                    if($request->has('dtdeleteList')) {
                                        //$getModules = doc_user_privilege::where('user_id',$request->user_id)->get();
                                        $getModuleschecked = doc_user_privilege::where('user_id',$request->user_id)->whereIn('module_id',$request->dtdeleteList )->where('active',1)->get();
                                        foreach ($getModuleschecked as $checked) {
                                            $update_priv = [
                                                'delete' => '1',
                                            ];
                                        ...
                                        $getModulesnotchecked = doc_user_privilege::where('user_id',$request->user_id)->whereNotIn('module_id',$request->dtdeleteList )->where('active',1)->get();
                                        foreach ($getModulesnotchecked as $notchecked) {
                                            $update_priv = [
                                                'delete' => '0',
                                            ];
                                            ...
                                            if($request->has('dtimportList')) {
                                                //$getModules = doc_user_privilege::where('user_id',$request->user_id)->get();
                                                $getModuleschecked = doc_user_privilege::where('user_id',$request->user_id)->whereIn('module_id',$request->dtimportList )->where('active',1)->get();
                                                foreach ($getModuleschecked as $checked) {
                                                    $update_priv = [
                                                        'import' => '1',
                                                    ];
                                                ...
                                                $getModulesnotchecked = doc_user_privilege::where('user_id',$request->user_id)->whereNotIn('module_id',$request->dtimportList )->where('active',1)->get();
                                                foreach ($getModulesnotchecked as $notchecked) {
                                                    $update_priv = [
                                                        'import' => '0',
                                                    ];
                                                    ...
                                                    if($request->has('dtexportList')) {
                                                        //$getModules = doc_user_privilege::where('user_id',$request->user_id)->get();
                                                        $getModuleschecked = doc_user_privilege::where('user_id',$request->user_id)->whereIn('module_id',$request->dtexportList )->where('active',1)->get();
                                                        foreach ($getModuleschecked as $checked) {
                                                            $update_priv = [
                                                                'export' => '1',
                                                            ];
                                                        ...
                                                        $getModulesnotchecked = doc_user_privilege::where('user_id',$request->user_id)->whereNotIn('module_id',$request->dtexportList )->where('active',1)->get();
                                                        foreach ($getModulesnotchecked as $notchecked) {
                                                            $update_priv = [
                                                                'export' => '0',
                                                            ];
                                                            ...
                                                            if($request->has('dtprintList')) {
                                                                //$getModules = doc_user_privilege::where('user_id',$request->user_id)->get();
                                                                $getModuleschecked = doc_user_privilege::where('user_id',$request->user_id)->whereIn('module_id',$request->dtprintList )->where('active',1)->get();
                                                                foreach ($getModuleschecked as $checked) {
                                                                    $update_priv = [
                                                                        'print' => '1',
                                                                    ];
                                                                ...
                                                                $getModulesnotchecked = doc_user_privilege::where('user_id',$request->user_id)->whereNotIn('module_id',$request->dtprintList )->where('active',1)->get();
                                                                foreach ($getModulesnotchecked as $notchecked) {
                                                                    $update_priv = [
                                                                        'print' => '0',
                                                                    ];
    ...
    public function load_accounts(Request $request){
        ...
        if($request->filter_data === "Active"){
            ...
            //$accounts = tblemployee::with('getHRdetails','getUsername')->has('getUsername')->where('agencyid',$request->filter_data)->where('active',1)->first();
            ...
            $accounts = User::with('getUserinfo')->where('active',1)->whereNull('employee')->get();
        ...
        foreach ($accounts as $dt) {
            ...
            if($get_user_priv){
                if($get_user_priv[0]->delete == 1 || Auth::user()->role_name == 'Admin'){
                    $can_delete = '<a id="btn_delete_account" href="javascript:;" class="dropdown-item" data-ac-id="'.$dt->id.'"> <i class="fa fa-trash-alt w-4 h-4 mr-2 text-danger"></i> Delete </a>';
                }
            }else{
                $can_delete = '<a id="btn_delete_account" href="javascript:;" class="dropdown-item" data-ac-id="'.$dt->id.'"> <i class="fa fa-trash-alt w-4 h-4 mr-2 text-danger"></i> Delete </a>';
            }
    ...
    public function load_user_id(Request $request){
        ...
        if($request->has('user_id')) {
            $user = User::with('get_user_employment.get_employment_status','get_user_profile')->where('id',$request->user_id)->first();

        }else{
            $user = User::with('get_user_employment.get_employment_status','get_user_profile')->where('id',Auth::user()->id)->first();
        }
        ...
        if( $user->get_user_profile()->exists()){
            ...
            __notification_set(1,$full_name,'Details loaded!');
        }else{
            if($get_user_priv){
                if( $get_user_priv[0]->create == 1 ||  Auth::user()->role_name == 'Admin' ){
        ...
        $accounts_Applicants = User::with('getUserinfo')->where('active',1)->whereNull('employee')->get()->count();
    ...
    public function sync_data_account_profile_employee(Request $request){
        ...
        foreach($users as $user){
            $get_profile_agency_id = tblemployee::where('agencyid',$user->employee)->first();
            $get_update_employment_agency_id = agency_employees::where('agency_id',$user->employee)->first();
            ...
            if($get_update_employment_agency_id){
                $agency_id = $get_update_employment_agency_id->id;

            }
            if($get_profile_agency_id){
                $pofile_id = $get_profile_agency_id->id;

                $firstname = $get_profile_agency_id->firstname;
                $lastname = $get_profile_agency_id->lastname;
                $middlename = $get_profile_agency_id->mi;
            }
            ...
            tblemployee::where('agencyid',$user->employee)->update($update_profile);

            $update_employment = [
                'user_id' => $user->id,
                'profile_id' => $pofile_id,
            ];

            agency_employees::where('agency_id',$user->employee)->update($update_employment);
            ...
            User::where('id', $user->id)->update($update_account);
        }
    ...
    public function sync_data_account_profile_employee_temp(Request $request){
        ...
        $profiles = tblemployee::with('getHRdetails')->whereNotNull('agencyid')->where('active',1)->get();
        ...
        foreach($profiles as $profile){
            ...
            if($check_if_already_in){
                ...
                $agency_code = default_setting::where('key','agency_code')->first();
    ...
    public function manage_load_edit_profile_id(Request $request){
        ...
        if($request->has('profile_id')) {
            ...
            <a id="btn_print_PDS" target="_blank" href="/my/print/pds/'.Crypt::encrypt($load_profile->user_id).'" class="ml-auto text-primary truncate flex items-center"> <i class="fa fa-print w-4 h-4 mr-2"></i> PDS </a>';
