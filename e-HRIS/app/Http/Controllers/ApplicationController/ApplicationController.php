...
class ApplicationController extends Controller
{
    ...
    public function load_applicants()
    {
        ...
        $applicants = applicants_list::with([
                    'get_profile', 'get_job_info', 'get_status'])
                    ->where('active', true)
        ...
        if($applicants)
        {
            foreach ($applicants as $dt)
            {
                ...
                if($profile)
                {
                    ...
                    {
                        $profile_pic = url('') . "/uploads/applicants/dssc_logo.png";
                    }else
                    {
                        $profile_pic = url('') . "/uploads/applicants/" . $profile->image;
                    }
                }else
                {
                    $lastname = "N/A";
    ...
    public function get_job_attachments(Request $request)
    {
        ...
        if ($attachments)
        {
            foreach ($attachments as $dt)
            {
                ...
                $attachment_id = $dt->id;
                $attachment_path = $dt->attachment_path;
...
//                        ->where('applicant_status', 1)
                        ->where('active', 1)
                        ->get();
//        $positions = applicants_attachments::with('get_job_info.get_Position')->where('jobref_no', $request->position_ref_num)->where('applicant_id', $request->applicant_id)->get();
            ...
            foreach ($positions as $post)
            {
                if($post->get_job_info)
                {
                    ...
                    $applied_for .=
                        "<tr>
                                <td>
                                    <a href='javascript:;' class='flex underline decoration-dotted items-center mt-2 btn_applied_position' data-applicant-id=".$applicant_id." data-pos-ref=".$job_ref_no."> <i class='fa-solid fa-circle-dot w-2 h-2 mr-2'></i>$position </a>
                                </td>
                                <td>
                                    <div class='flex justify-center items-center whitespace-nowrap text-".$status_class."'><div class='w-2 h-2 bg-".$status_class." rounded-full mr-3'></div>$status_name</div>
                                </td>
    ...
    {
        $get_attachments = applicants_attachments::where('attachment_path', $path)->get();
        ...
        $pathToFile = $storagePath."public/applicant/attachments/".$path."/".$fileName;
    ...
    public function get_available_positions(Request $request)
    {
        ...
        $get_job_info = tbljob_info::with(['get_profile', 'get_remarks', 'get_job_doc_requirements', 'get_educ_requirements', 'get_SG', 'get_Position', 'get_position_type'])
            ->where('active', true )
            ->Where('status', 14 )
            ->orWhere('status', 13 )
            ->latest('id')
...
//                $destinationPath = 'uploads/applicants/'.$folder;
