            ...
            if(data.length > 0){
                ...
                for(var i=0;i<data.length;i++)
                {
                    ...
                    '<tr>'+
                            ...
                            '<a id="'+ref_num+'" target="_blank" href="/hiring/print-position-hiring/'+ref_num+'" class="w-8 h-8 rounded-full flex items-center justify-center border dark:border-darkmode-400 ml-2 text-slate-400 zoom-in tooltip" title="Update"><i class="icofont-print text-success"></i></a>'+
...
function get_competency()
{
    $("#competency").on("change",function()
    {
        ...
        $.ajax({
            ...
            type: "GET",
            url: bpath = '/hiring/get-competency',
            data:{_token,competency_id},
            dataType: "json",

            success:function(response)
            {
                let competency = JSON.stringify(response).slice(10).replace(/[0-9`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi,'');
                console.log(competency);
                $("#competency_details").val(competency);
            },
            error: function(xhr, status, error) {
                console.log("Error in ajax: " + status + "\nError: " + error + "\nError detail: " + xhr.responseText);
            } ,
...
//responsible for the clicking of the close or open button
