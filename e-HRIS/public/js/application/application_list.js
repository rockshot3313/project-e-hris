...
$("body").on('click', '#btn_toggle_application_modal', function (){
    ...
    //         {
    //             __notif_show(1, "Success", "application approved successfully!")
    //             load_applicants_datatable();
    //         }
    //
    //     }
...
$("body").on('click', '.btn_approve', function (){
    ...
    approve_button(job_ref_no, applicant_id, action_type);

});

function approve_button(job_ref_no, applicant_id, action_type){

    $("body").on('click', '#btn_submit_approval', function (){

        let application_note = $('#application_note').val();

        approve(job_ref_no, applicant_id, action_type, application_note);
    });
}

function approve(job_ref_no, applicant_id, action_type, application_note){

    $.ajax({
        url: bpath + 'application/approve',
        type: "POST",
        data: {
            _token, job_ref_no, applicant_id, action_type, application_note,
        },
        success: function (response) {
...
$("body").on('click', '.btn_disapprove', function (){
    ...
    __dropdown_close("#application_approve_dd");
    disapprove_button(job_ref_no, applicant_id, action_type);

});

function disapprove_button(job_ref_no, applicant_id, action_type){

    $("body").on('click', '#btn_submit_approval', function (){

        let application_note = $('#application_note').val();

        disapprove(job_ref_no, applicant_id, action_type, application_note);
    });
}

function disapprove(job_ref_no, applicant_id, action_type, application_note){
