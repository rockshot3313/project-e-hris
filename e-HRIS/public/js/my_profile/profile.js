        ...
        success: function(response) {
            ...
            if(data.length > 0) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    $('#current_profile_picture_value').val(profile_pic);
            ...
            success: function(response) {
                ...
                if(data.length > 0) {
                    for (var i = 0; i < data.length; i++) {
                        ...
                        let mobile_number = data[i]['telephone'];
                        let username = data[i]['email'];
                        let password = data[i]['password'];

                        let full_name = first_name+" "+last_name;

                        let html_data_1 = '<div class="sm:whitespace-normal font-medium text-lg">'+first_name+" "+mid_name+" "+last_name+'</div>'+
...
function educational_background(){
    ...
    $("body").on('click', '.delete_saved_educ_bg', function (){
        ...
        $.ajax({
            url: bpath + 'my/remove/academic/background',
            type: "POST",
    ...
    $("body").on('click', '#update_educ_bg', function (){
        ...
        let academic_id =  $('#acad_id').val();
        let acad_level = $('#acad_level').val();

        let form_data = {
            _token,
            academic_id,
            acad_level : acad_level,
            school_name : $('#educ_school_name').val(),
            acad_from : $('#educ_school_from').val(),
            acad_to : $('#educ_school_to').val(),
            acad_course : $('#educ_degree_course').val(),
            acad_earned : $('#educ_highest_level_earned').val(),
            acad_graduated : $('#educ_year_graduated').val(),
            acad_scholar : $('#educ_scholarship').val(),
        }

        $.ajax({
            url: bpath + 'my/update/academic/background',
            type: "POST",
            data: form_data,
            success: function (response) {
                ...
                load_educational_bg();
                __notif_show(1, 'Success', 'Academic updated successfully!');
            }
        });
...
function civil_service(){
    ...
    $("body").on('click', '#add_civil_service', function (){
        ...
        $('#save_civil_service').show();
        $('#update_cs').hide();

    });

    $("body").on('click', '#save_civil_service', function (){
        add_row_civil_service();
    });
    ...
    function add_row_civil_service(){
        ...
        var tr='<tr class="hover:bg-gray-200">'+
            ...
            '<td style="text-transform:uppercase"><input type="text" style="display: none" name="td_cs_date_validity[]" class="form-control" value="'+$('#cs_date_validity').val()+'"></td>'+
            '<td>' +
                '<div class="flex justify-center items-center">' +
                    '<a href="javascript:void(0);" class="delete text-center"><i  class="w-4 h-4 text-danger fa-solid fa-trash"></i></a>' +
                '</div>'+
...
function edit_saved_work_exp(){
    ...
    $("body").on('click', '.edit_work_exp', function (){
        ...
        let work_service = $(this).data('exp-service');
        ...
        $('#work_exp_govt_service').val(work_service);
...
function edit_saved_vol_work(){
    ...
    $("body").on('click', '.edit_vol_work', function (){
        ...
        let vol_work_name = $(this).data('vol-orgname');
        let vol_work_from = $(this).data('vol-from');
        let vol_work_to = $(this).data('vol-to');
        let vol_work_hrs_num = $(this).data('vol-hrsnum');
    ...
    $('#ld_type').on('select2:select', function (e) {
        ...
        if($('#ld_type').val()=="OTHERS")
        {
            ...
            let html_data = '<label for="validation-form-1" class="form-label w-full flex flex-col sm:flex-row"> if Others <span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-slate-500">(Write in full if Others)</span> </label>' +
                            '<input id="ld_type_others" type="text" name="ld_sponsored_by" style="text-transform:uppercase" class="form-control" placeholder="Types of LD" minLength="2" required autoComplete="off">';
...
//         let ld_hrsnum = $(this).data('ld-hrsnum');
//         let ld_type = $(this).data('ld-type');
//         let ld_sponsor = $(this).data('ld-sponsor');
//
//         let new_type = '';
//
//         const child_mdl = tailwind.Modal.getOrCreateInstance(document.querySelector('#add_LD_mdl'));
//         child_mdl.toggle();
//
//         if (ld_type === "MANAGERIAL")
//         {
//             new_type = "MANAGERIAL";
//             $('#div_if_others').hide();
//
//         }else if (ld_type === "SUPERVISORY")
//         {
//             new_type = "SUPERVISORY";
//             $('#div_if_others').hide();
//
//         }else if (ld_type === "TECHNICAL")
//         {
//             new_type = "TECHNICAL";
//             $('#div_if_others').hide();
//
...
//         $('#ld_hr_number').val(ld_hrsnum);
//         $('#ld_type').val(ld_type);
//         $('#ld_sponsored_by').val(ld_sponsor);
//
//         $('#ld_type').val(new_type).trigger('change');
//
//         $('#ld_type').select2({
